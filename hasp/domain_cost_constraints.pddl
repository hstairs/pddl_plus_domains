(define
	...
	(previous_time_step) ;; previous time elapsed [s]
	(total_cost) ;; total cost of plan
	(cost_static_obstacle) ;; the immediate cost of the HAPS stepping into a static obstacle
	(cost_moving_obstacle) ;; the imeediate cost of the HAPS stepping into a moving obstacle
   ...
   (:process obstacles_shift
    ...
    ;; increase cost if HAPS steps into a static obstacle
    (:process increase_cost_static_obstacle
	:parameters (?static_obs -static_obstacle)
	:precondition (and
				(< relative_position_north (+ (bound_north ?static_obs) 50))
				(> relative_position_north (+ (bound_south ?static_obs) 50))
				(< relative_position_east (+ (bound_east ?static_obs) 50))
				(> relative_position_east (+ (bound_west ?static_obs) 50))	
	)
	:effect (and
		 	(increase (total-cost) cost_static_obstacle)
	)
    )
	
    ;; increase cost if HAPS steps into a moving obstacle
    (:process increase_cost_moving_obstacle
	:parameters (?moving_obs -moving_obstacle)
	:precondition (and
				(< relative_position_north (+ (bound_north ?moving_obs) 30))
				(> relative_position_north (+ (bound_south ?moving_obs) 30))
				(< relative_position_east (+ (bound_east ?moving_obs) 30))
				(> relative_position_east (+ (bound_west ?moving_obs) 30))
	)
	:effect (and
			(increase (total-cost) cost_moving_obstacle)
	)
    )
