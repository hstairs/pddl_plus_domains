;; Enrico Scala (enricos83@gmail.com) and Miquel Ramirez (miquel.ramirez@gmail.com)
(define (domain car)
;(:requirements :typing :durative-actions :fluents :time :negative-preconditions :duration-inequalities )
(:predicates (landed) (landing) (thrusting) (stop))
(:functions (M) (q) (d) (g) (v) (M_min) (ISP) (d_final) (v_margin) (d_margin) (falling-time) (thrust-duration))




(:process falling
    :parameters ()
    :precondition (and
                    (< (falling-time) 40)
                    (landing)
                    (< (d) (d_final))
                  )
    :effect (and 
		        (increase (d) (* #t (* 0.5 (v)) ) )
		        (increase (v) (* #t (g)))
		        (increase (falling-time) (* #t 1.0))
		    )
)

(:action start_descent
    :parameters()
    :precondition (and
                      (stop)
                  )
    :effect (and
                (landing)
                (assign (falling-time) 0)
                (not (stop))
            )
)

(:action land
    :parameters ()
    :precondition (and 
                        (landing)
                        (< (v) (v_margin)) (< (d) (d_final)) (> (d) (- (d_final) (d_margin)))  
                  )
    :effect (and
                (landed)
                (not (landing))
            )
)
		        
(:action start-thrust
    :parameters ()
    :precondition (and 
                        (not (thrusting))
                        (landing)
                  )
    :effect (and 
                (thrusting)
                (assign (thrust-duration) 0)
            )
)

(:action stop-thrust
    :parameters ()
    :precondition (and (thrusting))
    :effect (and
               (not (thrusting))
            )
)

(:process thrust
:parameters ()
:precondition (and 
                (< (thrust-duration) (/ (- (M) (M_min)) q) )
                (landing)
                (thrusting)
              )
:effect (and 
             (decrease (v) (* #t (* (* (ISP) (g)) (/ (q) (M))))) 
	         (decrease (M) (* #t (q)))	
	         (increase (thrust-duration) (* #t 1.0))
	    )
)



(:constraint anti-crash
    :parameters ()
    :condition (and (< (d) (d_final))(> (M) (M_min)))
)

)

